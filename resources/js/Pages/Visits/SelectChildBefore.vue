    ...
    <div>
        ...
        <div class="mt-1 relative">
            ...
            <ul v-show="selected == null  " tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3" class="overflow-visible max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5  focus:outline-none sm:text-sm">
                ...
                <span v-if="selected == child.id" class="absolute inset-y-0 right-0 flex items-center pr-4">
                    <!-- Heroicon name: check -->
...
export default {
    ...
    methods:{
        ...
        filter(){
            ...
            this.filtered = this.children.filter(child => child.name.toLowerCase().includes(this.search.toLowerCase()));
        },
        select(child){
            this.selected = child.id
            this.search = child.name
            this.setChildForNewVisit(child);
        },
    },
    mounted(){
        axios.get('/children/list')
       .then( data =>{
